...
let prices=trie();
  for (let i = 0; i < prices.length; i++){
    if(allcolor.length === 0){
        prixnet.push(prices[i].compare_at_price / 100);
       }else{
...
let element = document.querySelector('[class*="product-detail"] [class*="price product-price"] span[class*="money"]:not([class*="compare"])').textContent.trim().match(/\D/)[0]
    function replaceValues(array, newValue) {
        return array.map(() => newValue);
    }
 replaceValues(prixnet,element)
